...
import { ArrowLeft, CheckCircle, Mail, Phone, Briefcase, ChevronDown, ChevronUp } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

export default function ShortlistedReview() {
    ...
    if (isLoading || authLoading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-emerald-600 font-bold text-lg animate-pulse">Loading shortlisted candidates...</div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 mb-8 flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-2xl font-extrabold text-emerald-700">{applications.length}</p>
                        <p className="text-emerald-600 font-medium">
                            Shortlisted candidates across {Object.keys(grouped).length} job role{Object.keys(grouped).length !== 1 ? 's' : ''}
                        </p>
                    </div>
                </div>
                ...
                {applications.length === 0 ? (
                    <div className="text-center py-20 text-gray-400">
                        ...
                        <p className="text-xl font-semibold">No shortlisted candidates</p>
                        <p className="text-sm mt-1">No candidates have been shortlisted yet.</p>
                    </div>
                ) : (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            ...
                                            <p className="text-sm text-gray-500">{apps.length} shortlisted candidate{apps.length !== 1 ? 's' : ''}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                ...
                                {expandedJob === role && (
                                    <div className="border-t border-gray-100 overflow-x-auto">
                                        <table className="w-full text-left border-collapse text-sm">
                                            ...
                                            <tbody>
                                                {apps.map((app) => (
                                                    <tr key={app.id} className="border-b border-gray-100 hover:bg-emerald-50 transition-colors">
